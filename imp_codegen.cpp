...
//	cout << "Type checking OK" << endl;
	...
	cout << "Max stack size: " << typechecker.max_sp << endl;
	return;
}

void ImpCodeGen::visit(Program *p) {
	codegen(nolabel, "alloc", typechecker.mem_locals);
	p->body->accept(this);
...
void ImpCodeGen::visit(VarDeclaration *vd) {
...
void ImpCodeGen::visit(CommentVarDec *c) {
	return;
}
...
void ImpCodeGen::visit(CommentStatement *s) {
	return;
}

void ImpCodeGen::visit(Comment* c) {
	return;
}
...
//			break;
//		default:
//			cout << "unop " << Exp::unopToString(e->op) << " not implemented" << endl;
//	}
//	codegen(nolabel, op);
	return 0;
}
